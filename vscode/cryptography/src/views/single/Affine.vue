      ...
      <div class="panel left-panel">
        <form @submit.prevent="handleSubmit">
          <div class="form-group">
            <label for="keyA">KeyA:</label>
            <input type="text" id="keyA" v-model="Affine.keyA" placeholder="Enter KeyA" />
          </div>
          <div class="form-group">
            <label for="keyB">KeyB:</label>
            <input type="text" id="keyB" v-model="Affine.keyB" placeholder="Enter KeyB" />
          </div>
          <div class="form-group">
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="submitConfig">
          <div class="form-group">
            ...
            <input type="text" id="localIp" v-model="Affine.sourceIp" placeholder="Enter local IP" />
          </div>
          <div class="form-group">
            <label for="targetIp">目标IP:</label>
            <input type="text" id="targetIp" v-model="Affine.targetIp" placeholder="Enter target IP" />
          </div>
          <div class="form-group">
            <label for="port">端口:</label>
            <input type="number" id="port" v-model="Affine.port" placeholder="Enter port" />
          </div>
...
<script>
import { AffineEncrypt,AffineDecrypt,AffineEncryptFile,AffineDecryptFile } from '@/api/Affine.js';
...
export default {
  ...
  clearInputs() {
      this.Affine.plaintext = "";
      this.Affine.plainFile = "";
      this.Affine.ciphertext = "";
      this.Affine.cipherFile = "";
    ...
    async encrypt(){
      ...
      this.Affine.plaintext = document.getElementById("input").value;
      if(this.Affine.keyA === "" || this.Affine.keyB === ""){
        this.$message.error("请输入密钥");
        return;
      }
      if (!Number.isInteger(parseInt(this.Affine.keyA)) || !Number.isInteger(parseInt(this.Affine.keyB))) {
        this.$message.error("密钥必须是整数");
        return;
      }
      if (this.Affine.keyA < 1 || this.Affine.keyA > 25 || this.gcd(this.Affine.keyA, 26) !== 1) {
        this.$message.error("密钥A无效，必须在1到25之间并与26互质");
        return;
      }
      if (this.Affine.keyB < 0 || this.Affine.keyB > 25) {
        this.$message.error("密钥B无效，必须在0到25之间");
        return;
      }
        let res = await AffineEncrypt(this.Affine);
        if(res.code === 0){
       ...
       document.getElementById("output").value = res.data.ciphertext;
        ...
        this.Affine.plainFile = path;
        //console.log(this.Affine.plainFile);
        let res = await AffineEncryptFile(this.Affine);
        if(res.code === 0){
         this.$message.success(res.message);
       }else{
          this.$message.error(res.message);
    ...
    async decrypt(){
      if(this.selectedFile === null){
      this.Affine.ciphertext = document.getElementById("input").value;
      this.Affine.plaintext = null;
      if(this.Affine.keyA === "" || this.Affine.keyB === ""){
        this.$message.error("请输入密钥");
        return;
      }
      let res = await AffineDecrypt(this.Affine);
      if(res.code === 0){
        this.Affine.plaintext = res.data.plaintext;
        this.$message.success(res.message);
      }else{
        this.$message.error(res.message);
      }
      document.getElementById("output").value = this.Affine.plaintext;
    }else{
      if(this.Affine.keyA === "" || this.Affine.keyB === ""){
        this.$message.error("请输入密钥");
        return;
     ...
     if (this.Affine.keyB < 0 || this.Affine.keyB > 25) {
      ...
      let path = await this.uploadFile();
      this.Affine.cipherFile = path;
      this.Affine.plainFile =  null;
      //console.log(this.Affine.cipherFile);
      let res = await AffineDecryptFile(this.Affine);
      if(res.code === 0){
        this.Affine.plainFile = res.data.plainFile;
        this.$message.success(res.message);
      }else{
        this.$message.error(res.message);
      }
      document.getElementById("output").value = this.Affine.plainFile;
    }
    this.Affine.plaintext = "";
    this.Affine.plainFile = "";
    this.Affine.ciphertext = "";
