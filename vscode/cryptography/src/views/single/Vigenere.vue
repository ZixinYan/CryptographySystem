      ...
      <div class="panel left-panel">
        <form @submit.prevent="handleSubmit">
          <div class="form-group">
            ...
            <input type="text" id="key" v-model="Vigenere.key" placeholder="Enter Key" />
          </div>
          <div class="form-group">
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="submitConfig">
          <div class="form-group">
            ...
            <input type="text" id="localIp" v-model="Vigenere.sourceIp" placeholder="Enter local IP" />
          </div>
          <div class="form-group">
            <label for="targetIp">目标IP:</label>
            <input type="text" id="targetIp" v-model="Vigenere.targetIp" placeholder="Enter target IP" />
          </div>
          <div class="form-group">
            <label for="port">端口:</label>
            <input type="number" id="port" v-model="Vigenere.port" placeholder="Enter port" />
          </div>
...
<script>
import { VigenereEncrypt,VigenereDecrypt,VigenereEncryptFile,VigenereDecryptFile } from '@/api/Vigenere.js';
...
export default {
  ...
  clearInputs() {
      this.Vigenere.plaintext = "";
      this.Vigenere.plainFile = "";
      this.Vigenere.ciphertext = "";
      this.Vigenere.cipherFile = "";
      this.selectedFile = null;
    ...
    handleSubmit() {
      ...
      //console.log("提交成功: ", this.Vigenere);
    },
    ...
    async encrypt(){
      if(this.Vigenere.key === "" ){
        this.$message.error("请输入密钥");
        return;
      }
      const regex = /^[A-Za-z]+$/;
      if (!regex.test(this.Vigenere.key)) {
        this.$message.error("密钥必须只包含字母（A-Z，a-z）");
        return;
      }
      if(this.selectedFile === null){
        this.Vigenere.plaintext = document.getElementById("input").value;
        let res = await VigenereEncrypt(this.Vigenere);
        if(res.code === 0){
        ...
        this.Vigenere.plainFile = path;
        //console.log(this.Vigenere.plainFile);
        let res = await VigenereEncryptFile(this.Vigenere);
        if(res.code === 0){
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
    ...
    async decrypt(){
      ...
      if(this.selectedFile === null){
        this.Vigenere.ciphertext = document.getElementById("input").value;
        this.Vigenere.plaintext = null;
        let res = await VigenereDecrypt(this.Vigenere);
        if(res.code === 0){
          this.Vigenere.plaintext = res.data.plaintext;
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
        }
        document.getElementById("output").value = this.Vigenere.plaintext;
      }else{
        let path = await this.uploadFile();
        this.Vigenere.cipherFile = path;
        this.Vigenere.plainFile =  null;
       //console.log(this.Vigenere.cipherFile);
        let res = await VigenereDecryptFile(this.Vigenere);
        if(res.code === 0){
          this.Vigenere.plainFile = res.data.plainFile;
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
        }
        document.getElementById("output").value = this.Vigenere.plainFile;
      }
      this.Vigenere.plaintext = "";
      this.Vigenere.plainFile = "";
      this.Vigenere.ciphertext = "";
