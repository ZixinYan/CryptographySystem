      ...
      <div class="panel left-panel">
        <form @submit.prevent="handleSubmit">
          <div class="form-group">
            ...
            <input type="text" id="key" v-model="ColumnPermutation.key" placeholder="Enter Key" />
          </div>
          <div class="form-group">
      ...
      <div class="panel right-panel">
        <div class="section top-section">
          <div class="operations">
            <h1 id="title">ColumnPermutation加密解密</h1>
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="submitConfig">
          <div class="form-group">
            ...
            <input type="text" id="localIp" v-model="ColumnPermutation.sourceIp" placeholder="Enter local IP" />
          </div>
          <div class="form-group">
            <label for="targetIp">目标IP:</label>
            <input type="text" id="targetIp" v-model="ColumnPermutation.targetIp" placeholder="Enter target IP" />
          </div>
          <div class="form-group">
            <label for="port">端口:</label>
            <input type="number" id="port" v-model="ColumnPermutation.port" placeholder="Enter port" />
          </div>
...
<script>
import { ColumnPermutationEncrypt,ColumnPermutationDecrypt,ColumnPermutationEncryptFile,ColumnPermutationDecryptFile } from '@/api/ColumnPermutation.js';
...
export default {
  ...
  clearInputs() {
      this.ColumnPermutation.plaintext = "";
      this.ColumnPermutation.plainFile = "";
      this.ColumnPermutation.ciphertext = "";
      this.ColumnPermutation.cipherFile = "";
      this.selectedFile = null;
    ...
    handleSubmit() {
      ...
      //console.log("提交成功: ", this.ColumnPermutation);
    },
    ...
    async encrypt(){
      if(this.selectedFile === null){
        this.ColumnPermutation.plaintext = document.getElementById("input").value;
        if(this.ColumnPermutation.key === ""){
          this.$message.error("请输入密钥");
          return;
        }
        const regex = /^[A-Za-z]+$/;
        if (!regex.test(this.ColumnPermutation.key)) {
          this.$message.error("密钥必须只包含字母（A-Z，a-z）");
          return;
        }
        let res = await ColumnPermutationEncrypt(this.ColumnPermutation);
        if(res.code === 0){
       ...
       document.getElementById("output").value = res.data.ciphertext;
        ...
        this.ColumnPermutation.plainFile = path;
        //console.log(this.ColumnPermutation.plainFile);
        let res = await ColumnPermutationEncryptFile(this.ColumnPermutation);
        if(res.code === 0){
         this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
    ...
    async decrypt(){
      if(this.selectedFile === null){
        this.ColumnPermutation.ciphertext = document.getElementById("input").value;
        this.ColumnPermutation.plaintext = null;
        if(this.ColumnPermutation.key === ""){
          this.$message.error("请输入密钥");
          return;
        }
        ...
        let res = await ColumnPermutationDecrypt(this.ColumnPermutation);
        if(res.code === 0){
          this.ColumnPermutation.plaintext = res.data.plaintext;
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
        }
        document.getElementById("output").value = this.ColumnPermutation.plaintext;
      }else{
        if(this.ColumnPermutation.key === ""){
          this.$message.error("请输入密钥");
          return;
        ...
        let path = await this.uploadFile();
        this.ColumnPermutation.cipherFile = path;
        this.ColumnPermutation.plainFile =  null;
        //console.log(this.ColumnPermutation.cipherFile);
        let res = await ColumnPermutationDecryptFile(this.ColumnPermutation);
        if(res.code === 0){
          this.ColumnPermutation.plainFile = res.data.plainFile;
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
        }
        document.getElementById("output").value = this.ColumnPermutation.plainFile;
      }
      this.ColumnPermutation.plaintext = "";
      this.ColumnPermutation.plainFile = "";
      this.ColumnPermutation.ciphertext = "";
