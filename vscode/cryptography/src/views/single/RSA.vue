    ...
    <div class="container">
      ...
      <div class="panel left-panel">
        <form @submit.prevent="handleSubmit">
          <div class="form-group">
            ...
            <input type="text" id="publicKey" v-model="RSA.e" placeholder="Enter publicKey(E)" />
          </div>
          <div class="form-group">
            <label for="privateKey">Private Key:</label>
            <input type="text" id="privateKey" v-model="RSA.d" placeholder="Enter privateKey(D)" />
          </div>
          <div class="form-group">
            <label for="privateKey">N:</label>
            <input type="text" id="ciphertext" v-model="RSA.n" placeholder="Enter N" />
          </div>
          <button type="button" class="submit-btn" @click="openConfigModal()">获取密钥</button>
          <button type="button" class="submit-btn2" @click="encrypt()">加密！</button>
          <button type="button" class="submit-btn2" @click="decrypt()">解密！</button>
        </form>
      </div>
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="submitConfig">
          <div class="form-group">
            <label for="length">请选择密钥长度:</label>
            <select id="length" v-model="RSA.length">
          ...
          <button type="submit" class="submit-btn" @click="getkey()">确认</button>
...
<script>
import { RSAEncrypt,RSADecrypt,RSAEncryptFile,RSADecryptFile,RSAGenerateKey } from '@/api/RSA.js';
...
export default {
  ...
  startResizeHorizontal(event) {
    ...
    async getkey(){
      this.clearInputs();
      let res = await RSAGenerateKey(this.RSA);
      if(res.code === 0){
        document.getElementById("output").value = "Public Key(e): " + res.data.e +
        "\nPrivate Key(D):" + res.data.d + "\nN:" + res.data.n;
    ...
    async encrypt(){
      this.checkContent();
      if(this.RSA.e === "" || this.RSA.n === "" || this.RSA.d !== ""){
         this.$message.error("加密需要公钥e和N");
         return;
        }
      if(this.selectedFile === null){
      this.RSA.plaintext = document.getElementById("input").value;
        let res = await RSAEncrypt(this.RSA);
        if(res.code === 0){
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
       ...
       document.getElementById("output").value = res.data.ciphertext;
        ...
        this.RSA.plainFile = path;
        let res = await RSAEncryptFile(this.RSA);
        if(res.code === 0){
         this.$message.success(res.message);
       }else{
          this.$message.error(res.message);
    ...
    async decrypt(){
      this.checkContent();
      if(this.RSA.e !== "" || this.RSA.d === "" || this.RSA.n === ""){
        this.$message.error("验证需要私钥和密文对");
        return;
      }
      if(this.selectedFile === null){
      this.RSA.ciphertext = document.getElementById("input").value;
      let res = await RSADecrypt(this.RSA);
      if(res.code === 0){
        this.RSA.signature = res.data.signature;
        this.$message.success(res.message);
      }else{
        this.$message.error(res.message);
      }
      document.getElementById("output").value = res.data.plaintext;
    }else{
      let path = await this.uploadFile();
      this.RSA.plainFile =  path;
      let res = await RSADecryptFile(this.RSA);
      if(res.code === 0){
        this.RSA.signature = res.data.signature;
        this.$message.success(res.message);
      }else{
      ...
      document.getElementById("output").value = res.data.plainFile;
    }
    this.RSA.plaintext = "";
    this.RSA.plainFile = "";
    this.RSA.ciphertext = "";
