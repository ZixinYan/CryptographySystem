      ...
      <div class="panel left-panel">
        <form @submit.prevent="handleSubmit">
          <div class="form-group">
            ...
            <input type="text" id="key" v-model="RC4.key" placeholder="Enter Key" />
          </div>
          <div class="form-group">
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="submitConfig">
          <div class="form-group">
            ...
            <input type="text" id="localIp" v-model="RC4.sourceIp" placeholder="Enter local IP" />
          </div>
          <div class="form-group">
            <label for="targetIp">目标IP:</label>
            <input type="text" id="targetIp" v-model="RC4.targetIp" placeholder="Enter target IP" />
          </div>
          <div class="form-group">
            <label for="port">端口:</label>
            <input type="number" id="port" v-model="RC4.port" placeholder="Enter port" />
          </div>
...
<script>
import { RC4Encrypt,RC4Decrypt,RC4EncryptFile,RC4DecryptFile } from '@/api/RC4.js';
  ...
  clearInputs() {
    ...
    async encrypt(){
      if(this.selectedFile === null){
        this.RC4.plaintext = document.getElementById("input").value;
        if(this.RC4.key === "" ){
          this.$message.error("请输入密钥");
          return;
        }
        let res = await RC4Encrypt(this.RC4);
        if(res.code === 0){
        ...
        this.RC4.plainFile = path;
        //console.log(this.RC4.plainFile);
        let res = await RC4EncryptFile(this.RC4);
        if(res.code === 0){
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
    ...
    async decrypt(){
      if(this.selectedFile === null){
        this.RC4.ciphertext = document.getElementById("input").value;
        this.RC4.plaintext = null;
        if(this.RC4.key === "" ){
          this.$message.error("请输入密钥");
          return;
        }
        let res = await RC4Decrypt(this.RC4);
        if(res.code === 0){
          this.RC4.plaintext = res.data.plaintext;
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
        }
        document.getElementById("output").value = this.RC4.plaintext;
      }else{
        if(this.RC4.key === "" ){
          this.$message.error("请输入密钥");
          return;
        ...
        let path = await this.uploadFile();
        this.RC4.cipherFile = path;
        this.RC4.plainFile =  null;
       //console.log(this.RC4.cipherFile);
        let res = await RC4DecryptFile(this.RC4);
        if(res.code === 0){
          this.RC4.plainFile = res.data.plainFile;
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
        }
        document.getElementById("output").value = this.RC4.plainFile;
      }
      this.RC4.plaintext = "";
      this.RC4.plainFile = "";
      this.RC4.ciphertext = "";
