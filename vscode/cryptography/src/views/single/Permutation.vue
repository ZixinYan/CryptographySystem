      ...
      <div class="panel left-panel">
        <form @submit.prevent="handleSubmit">
          <div class="form-group">
            ...
            <input type="text" id="key" v-model="Permutation.key" placeholder="Enter Key" />
          </div>
          <div class="form-group">
      ...
      <div class="panel right-panel">
        <div class="section top-section">
          <div class="operations">
            <h1 id="title">Permutation加密解密</h1>
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="submitConfig">
          <div class="form-group">
            ...
            <input type="text" id="localIp" v-model="Permutation.sourceIp" placeholder="Enter local IP" />
          </div>
          <div class="form-group">
            <label for="targetIp">目标IP:</label>
            <input type="text" id="targetIp" v-model="Permutation.targetIp" placeholder="Enter target IP" />
          </div>
          <div class="form-group">
            <label for="port">端口:</label>
            <input type="number" id="port" v-model="Permutation.port" placeholder="Enter port" />
          </div>
...
<script>
import { PermutationEncrypt,PermutationDecrypt,PermutationEncryptFile,PermutationDecryptFile } from '@/api/Permutation.js';
import request from '@/utils/request.js';
import useTokenStore from '@/pinia/token.js'
const tokenStore = useTokenStore();
export default {
  name: "PermutationCipher",
  data() {
    return {
  ...
  clearInputs() {
      this.Permutation.plaintext = "";
      this.Permutation.plainFile = "";
      this.Permutation.ciphertext = "";
      this.Permutation.cipherFile = "";
      this.selectedFile = null;
    ...
    handleSubmit() {
      ...
      //console.log("提交成功: ", this.Permutation);
    },
    ...
    async encrypt(){
      if (!this.Permutation.key) {
        this.$message.error("请输入密钥");
        return;
      }
      const keyArray = this.Permutation.key.trim().split(" ").map(num => num.trim());
      const regex = /^[0-9]+$/;
      for (let num of keyArray) {
        if (!regex.test(num)) {
          this.$message.error("密钥必须是由数字组成的，数字之间用单个空格分隔");
          return;
       }
      }
      const numericKeys = keyArray.map(num => parseInt(num, 10));
      const minKey = Math.min(...numericKeys);
      if (minKey !== 1) {
        this.$message.error("密钥中的最小值必须为1");
       return;
      }
      const maxKey = Math.max(...numericKeys);
      const expectedKeys = new Set(Array.from({ length: maxKey - minKey + 1 }, (_, i) => minKey + i));
      const providedKeys = new Set(numericKeys);
      if (providedKeys.size !== expectedKeys.size || ![...expectedKeys].every(key => providedKeys.has(key))) {
        this.$message.error(`密钥必须包含从 1 到 ${maxKey} 的所有连续数字`);
       return;
     }
      const uniqueKeys = new Set(keyArray);
      if (uniqueKeys.size !== keyArray.length) {
        this.$message.error("密钥中的数字不能重复");
        return;
      }
      if(this.selectedFile === null){
        this.Permutation.plaintext = document.getElementById("input").value;
        let res = await PermutationEncrypt(this.Permutation);
        if(res.code === 0){
        ...
        this.Permutation.plainFile = path;
        //console.log(this.Permutation.plainFile);
        let res = await PermutationEncryptFile(this.Permutation);
        if(res.code === 0){
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
    ...
    async decrypt(){
      ...
      if(this.selectedFile === null){
        this.Permutation.ciphertext = document.getElementById("input").value;
        this.Permutation.plaintext = null;
        let res = await PermutationDecrypt(this.Permutation);
        if(res.code === 0){
          this.Permutation.plaintext = res.data.plaintext;
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
        }
        document.getElementById("output").value = this.Permutation.plaintext;
      }else{
        let path = await this.uploadFile();
        this.Permutation.cipherFile = path;
        this.Permutation.plainFile =  null;
       //console.log(this.Permutation.cipherFile);
        let res = await PermutationDecryptFile(this.Permutation);
        if(res.code === 0){
          this.Permutation.plainFile = res.data.plainFile;
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
        }
        document.getElementById("output").value = this.Permutation.plainFile;
      }
      this.Permutation.plaintext = "";
      this.Permutation.plainFile = "";
      this.Permutation.ciphertext = "";
