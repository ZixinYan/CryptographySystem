      ...
      <div class="panel left-panel">
        <form @submit.prevent="handleSubmit">
          <div class="form-group">
            ...
            <input type="text" id="key" v-model="Playfair.key" placeholder="Enter Key" />
          </div>
          <div class="form-group">
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="submitConfig">
          <div class="form-group">
            ...
            <input type="text" id="localIp" v-model="Playfair.sourceIp" placeholder="Enter local IP" />
          </div>
          <div class="form-group">
            <label for="targetIp">目标IP:</label>
            <input type="text" id="targetIp" v-model="Playfair.targetIp" placeholder="Enter target IP" />
          </div>
          <div class="form-group">
            <label for="port">端口:</label>
            <input type="number" id="port" v-model="Playfair.port" placeholder="Enter port" />
          </div>
...
<script>
import { PlayfairEncrypt,PlayfairDecrypt,PlayfairEncryptFile,PlayfairDecryptFile } from '@/api/Playfair.js';
...
export default {
  ...
  clearInputs() {
      this.Playfair.plaintext = "";
      this.Playfair.plainFile = "";
      this.Playfair.ciphertext = "";
      this.selectedFile = null;
      this.Playfair.cipherFile = "";
    ...
    handleSubmit() {
      ...
      //console.log("提交成功: ", this.Playfair);
    },
    ...
    async encrypt(){
      if(this.Playfair.key === "" ){
          this.$message.error("请输入密钥");
          return;
        }
        const regex = /^[A-Za-z]+$/;
        if (!regex.test(this.Playfair.key)) {
          this.$message.error("密钥必须只包含字母（A-Z，a-z）");
         return;
        }
      if(this.selectedFile === null){
        this.Playfair.plaintext = document.getElementById("input").value;
        let res = await PlayfairEncrypt(this.Playfair);
        if(res.code === 0){
        ...
        this.Playfair.plainFile = path;
        console.log(this.Playfair.plainFile);
        console.log(this.Playfair.key);
        let res = await PlayfairEncryptFile(this.Playfair);
        console.log(res);
        if(res.code === 0){
          this.$message.success(res.message);
        }else{
    ...
    async decrypt(){
      if(this.selectedFile === null){
        this.Playfair.ciphertext = document.getElementById("input").value;
        this.Playfair.plaintext = null;
        if(this.Playfair.key === "" ){
          this.$message.error("请输入密钥");
          return;
        }
        ...
        let res = await PlayfairDecrypt(this.Playfair);
        if(res.code === 0){
          this.Playfair.plaintext = res.data.plaintext;
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
        }
        document.getElementById("output").value = this.Playfair.plaintext;
      }else{
        if(this.Playfair.key === "" ){
          this.$message.error("请输入密钥");
          return;
        ...
        let path = await this.uploadFile();
        this.Playfair.cipherFile = path;
        this.Playfair.plainFile =  null;
       //console.log(this.Playfair.cipherFile);
        let res = await PlayfairDecryptFile(this.Playfair);
        if(res.code === 0){
          this.Playfair.plainFile = res.data.plainFile;
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
        }
        document.getElementById("output").value = this.Playfair.plainFile;
      }
      this.Playfair.plaintext = "";
      this.Playfair.plainFile = "";
      this.Playfair.ciphertext = "";
