      ...
      <div class="panel left-panel">
        <form @submit.prevent="handleSubmit">
          <div class="form-group">
            ...
            <input type="text" id="key" v-model="Autokey.key" placeholder="Enter Key" />
          </div>
          <div class="form-group">
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="submitConfig">
          <div class="form-group">
            ...
            <input type="text" id="localIp" v-model="Autokey.sourceIp" placeholder="Enter local IP" />
          </div>
          <div class="form-group">
            <label for="targetIp">目标IP:</label>
            <input type="text" id="targetIp" v-model="Autokey.targetIp" placeholder="Enter target IP" />
          </div>
          <div class="form-group">
            <label for="port">端口:</label>
            <input type="number" id="port" v-model="Autokey.port" placeholder="Enter port" />
          </div>
...
<script>
import { AutokeyEncrypt,AutokeyDecrypt,AutokeyEncryptFile,AutokeyDecryptFile } from '@/api/Autokey.js';
...
export default {
  ...
  clearInputs() {
      this.Autokey.plaintext = "";
      this.Autokey.plainFile = "";
      this.Autokey.ciphertext = "";
      this.Autokey.cipherFile = "";
    ...
    async encrypt(){
      ...
      this.Autokey.plaintext = document.getElementById("input").value;
      if(this.Autokey.key === "" ){
        this.$message.error("请输入密钥");
        return;
      }
      const regex = /^[A-Za-z]+$/;
      if (!regex.test(this.Autokey.key)) {
        this.$message.error("密钥必须只包含字母（A-Z，a-z）");
       return;
     }
      let res = await AutokeyEncrypt(this.Autokey);
      if(res.code === 0){
      ...
      document.getElementById("output").value = res.data.ciphertext;
        ...
        this.Autokey.plainFile = path;
        //console.log(this.Autokey.plainFile);
        let res = await AutokeyEncryptFile(this.Autokey);
        if(res.code === 0){
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
    ...
    async decrypt(){
      if(this.selectedFile === null){
      this.Autokey.ciphertext = document.getElementById("input").value;
      this.Autokey.plaintext = null;
      if(this.Autokey.key === "" ){
        this.$message.error("请输入密钥");
        return;
      }
      ...
      let res = await AutokeyDecrypt(this.Autokey);
      if(res.code === 0){
        this.Autokey.plaintext = res.data.plaintext;
        this.$message.success(res.message);
      }else{
        this.$message.error(res.message);
      }
      document.getElementById("output").value = this.Autokey.plaintext;
      }else{
        if(this.Autokey.key === ""){
          this.$message.error("请输入密钥");
          return;
        ...
        let path = await this.uploadFile();
        this.Autokey.cipherFile = path;
        this.Autokey.plainFile =  null;
       //console.log(this.Autokey.cipherFile);
        let res = await AutokeyDecryptFile(this.Autokey);
        if(res.code === 0){
          this.Autokey.plainFile = res.data.plainFile;
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
        }
        document.getElementById("output").value = this.Autokey.plainFile;
      }
      this.Autokey.plaintext = "";
      this.Autokey.plainFile = "";
      this.Autokey.ciphertext = "";
