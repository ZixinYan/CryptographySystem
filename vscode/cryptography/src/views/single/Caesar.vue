      ...
      <div class="panel left-panel">
        <form @submit.prevent="handleSubmit">
          <div class="form-group">
            ...
            <input type="text" id="key" v-model="Caesar.key" placeholder="Enter Key" />
          </div>
          <div class="form-group">
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="submitConfig">
          <div class="form-group">
            ...
            <input type="text" id="localIp" v-model="Caesar.sourceIp" placeholder="Enter local IP" />
          </div>
          <div class="form-group">
            <label for="targetIp">目标IP:</label>
            <input type="text" id="targetIp" v-model="Caesar.targetIp" placeholder="Enter target IP" />
          </div>
          <div class="form-group">
            <label for="port">端口:</label>
            <input type="number" id="port" v-model="Caesar.port" placeholder="Enter port" />
          </div>
...
<script>
import { CaesarEncrypt,CaesarDecrypt,CaesarEncryptFile,CaesarDecryptFile } from '@/api/Caesar.js';
...
export default {
  ...
  clearInputs() {
      this.Caesar.plaintext = "";
      this.Caesar.plainFile = "";
      this.Caesar.ciphertext = "";
      this.Caesar.cipherFile = "";
      this.selectedFile = null;
    ...
    async encrypt(){
      ...
      const regex = /^[A-Za-z]{1}$/; // 限制为单个字母
      if (!regex.test(this.Caesar.key)) {
        this.$message.error("密钥必须是单个字母（A-Z，a-z）");
        return;
      }
      if(this.selectedFile === null){
        this.Caesar.plaintext = document.getElementById("input").value;
        let res = await CaesarEncrypt(this.Caesar);
        if(res.code === 0){
        ...
        this.Caesar.plainFile = path;
        //console.log(this.Caesar.plainFile);
        let res = await CaesarEncryptFile(this.Caesar);
        if(res.code === 0){
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
    ...
    async decrypt(){
      ...
      if(this.selectedFile === null){
        this.Caesar.ciphertext = document.getElementById("input").value;
        this.Caesar.plaintext = null;
        let res = await CaesarDecrypt(this.Caesar);
        if(res.code === 0){
          this.Caesar.plaintext = res.data.plaintext;
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
        }
        document.getElementById("output").value = this.Caesar.plaintext;
      }else{
        let path = await this.uploadFile();
        this.Caesar.cipherFile = path;
        this.Caesar.plainFile =  null;
       //console.log(this.Caesar.cipherFile);
        let res = await CaesarDecryptFile(this.Caesar);
        if(res.code === 0){
          this.Caesar.plainFile = res.data.plainFile;
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
        }
        document.getElementById("output").value = this.Caesar.plainFile;
      }
      this.Caesar.plaintext = "";
      this.Caesar.plainFile = "";
      this.Caesar.ciphertext = "";
