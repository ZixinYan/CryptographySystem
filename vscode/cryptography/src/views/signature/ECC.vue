      ...
      <div class="panel left-panel">
        <form @submit.prevent="handleSubmit">
          <div class="form-group">
            ...
            <input type="text" id="publicKey" v-model="ECC.publicKey" placeholder="Enter publicKey" />
          </div>
          <div class="form-group">
            <label for="privateKey">Private Key:</label>
            <input type="text" id="privateKey" v-model="ECC.privateKey" placeholder="Enter privateKey" />
          </div>
          <div class="form-group">
            <label for="privateKey">Ciphertext:</label>
            <input type="text" id="ciphertext" v-model="ECC.ciphertext" placeholder="like: String1 String2" />
          </div>
          <button type="button" class="submit-btn" @click="getkey()">获取密钥</button>
          <button type="button" class="submit-btn2" @click="encrypt()">签名！</button>
          <button type="button" class="submit-btn2" @click="decrypt()">验证！</button>
        </form>
      </div>
...
<script>
import { ECCEncrypt,ECCDecrypt,ECCEncryptFile,ECCDecryptFile,ECCGenerateKey } from '@/api/ECC.js';
...
export default {
  ...
  startResizeHorizontal(event) {
    ...
    async getkey(){
      this.clearInputs();
      let res = await ECCGenerateKey();
      if(res.code === 0){
        document.getElementById("output").value = "public Key: " + res.data.publicKey +
        "\nPrivate Key:" + res.data.privateKey;
    ...
    async encrypt(){
      this.checkContent();
      if(this.ECC.publicKey === "" || this.ECC.privateKey !== "" || this.ECC.ciphertext !== ""){
         this.$message.error("签名仅需要公钥");
         return;
        }
      if(this.selectedFile === null){
      this.ECC.plaintext = document.getElementById("input").value;
        let res = await ECCEncrypt(this.ECC);
        if(res.code === 0){
          this.$message.success(res.message);
        }else{
          this.$message.error(res.message);
       ...
       document.getElementById("output").value = res.data.ciphertext;
        ...
        this.ECC.plainFile = path;
        //console.log(this.ECC.plainFile);
        let res = await ECCEncryptFile(this.ECC);
        if(res.code === 0){
         this.$message.success(res.message);
       }else{
          this.$message.error(res.message);
    ...
    async decrypt(){
      ...
      if(this.selectedFile === null){
      this.ECC.plaintext = document.getElementById("input").value;
      let res = await ECCDecrypt(this.ECC);
      if(res.code === 0){
        this.ECC.signature = res.data.signature;
        this.$message.success(res.message);
      }else{
        this.$message.error(res.message);
      }
      if(this.ECC.signature === "0"){
        document.getElementById("output").value = "验证成功";
      }else{
        document.getElementById("output").value = "验证失败";
      }
    }else{
      let path = await this.uploadFile();
      this.ECC.plainFile =  path;
      let res = await ECCDecryptFile(this.ECC);
      if(res.code === 0){
        this.ECC.signature = res.data.signature;
        this.$message.success(res.message);
      }else{
